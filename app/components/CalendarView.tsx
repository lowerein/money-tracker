...
export default async function CalendarView({ userId, year, month, selectedDay, viewUserIds, sharedUsers, currentUser }: CalendarViewProps) {
  const firstDay = new Date(year, month, 1)
  const lastDay = new Date(year, month + 1, 0)
  
  const expenses = await prisma.expense.findMany({
    where: { userId: { in: viewUserIds }, date: { gte: firstDay, lte: lastDay } },
    include: { user: true }
  })
  ...
  const dailyExpenses = expenses.reduce((acc, exp) => {
    ...
    if (!acc[dateStr][exp.userId]) acc[dateStr][exp.userId] = { name: exp.user.name, total: 0 }
    acc[dateStr][exp.userId].total += exp.amount
    return acc
  }, {} as Record<string, Record<string, { name: string | null, total: number }>>)

  // 🌟 改變整理結構：儲存 { emoji, name } 而唔係單純 string
  const dailyHabits = habitLogs.reduce((acc, log) => {
    const d = new Date(log.date)
    const dateStr = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`
    
    const isCompleted = log.habit.type === "BOOLEAN" 
      ? log.value > 0 
    ...
    if (isCompleted) {
      ...
      acc[dateStr][log.userId].habits.push({ emoji: log.habit.emoji, name: log.habit.name })
    }
    return acc
  }, {} as Record<string, Record<string, { name: string | null, habits: { emoji: string, name: string }[] }>>)
    ...
    <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden transition-colors">
      ...
      <div className="grid grid-cols-7">
        {calendarGrid.map((day, index) => {
          if (day === null) return <div key={`empty-${index}`} className="h-24 border-b border-r border-gray-50 dark:border-gray-800 bg-gray-50/30 dark:bg-gray-800/20" />
          ...
          const userIdsWithExpenses = Object.keys(dayExpensesData).sort((a, b) => {
            if (a === currentUser.id) return -1
            if (b === currentUser.id) return 1
            return 0
          })
                ...
                {userIdsWithExpenses.length > 0 ? (
                  <>
                    {userIdsWithExpenses.slice(0, 2).map(uid => {
                      ...
                      return (
                        <div key={uid} className="flex items-center w-full justify-center" title={data.name || "朋友"}>
                          {!isMe && <span className="bg-red-100 dark:bg-red-900/80 text-red-600 dark:text-red-300 rounded-full w-3 h-3 flex items-center justify-center mr-1 font-bold text-[8px] shrink-0">{(data.name || "F").charAt(0).toUpperCase()}</span>}
                          <span className={`text-[10px] font-black leading-none truncate ${isSelected ? 'text-blue-700 dark:text-blue-300' : (isMe ? 'text-red-500 dark:text-red-400' : 'text-gray-500 dark:text-gray-400')}`}>${data.total.toFixed(0)}</span>
                        </div>
                      )
                    })}
                    {userIdsWithExpenses.includes(currentUser.id) && (
                      <div className="w-full h-[3px] mt-0.5 bg-gray-100 dark:bg-gray-700 rounded-full max-w-[30px] overflow-hidden">
                        <div className={`h-full ${isSelected ? 'bg-blue-400' : 'bg-red-400 dark:bg-red-500'}`} style={{ width: `${Math.min((dayExpensesData[currentUser.id].total / 500) * 100, 100)}%` }} />
                      </div>
                    )}
                  </>
