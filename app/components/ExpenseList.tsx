...
export default async function ExpenseList({
  ...
  const expenses = await prisma.expense.findMany({
    ...
    orderBy: [
      {
        category: {
          order: "asc", // 1. 先跟隨分類設定的次序
        },
      },
      {
        createdAt: "desc", // 2. 同一分類內，最新的放上面
      },
    ],
  ...
  const viewCategories = await prisma.category.findMany({
    ...
    // 🌟 修改呢行：先跟 order 排，再跟 createdAt 排
    orderBy: [{ order: "asc" }, { createdAt: "asc" }],
  });
  // 🌟 去除重複名稱嘅分類 (因為你同朋友可能有同名分類)
  const uniqueCategories = Array.from(
    new Map(viewCategories.map((c) => [c.name, c])).values(),
  );
  ...
  const catQuery =
    selectedCats.length > 0
      ? `&cat=${encodeURIComponent(selectedCats.join(","))}`
      : "";
      ...
      <div className="px-6 py-4 bg-gray-50/50 dark:bg-gray-800/30 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
        <div className="flex flex-col">
          <span className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em]">
            {day > 0 ? `${month + 1}月${day}日` : `${month + 1}月`}{" "}
            {selectedCats.length > 0 ? "篩選開支" : "總開支"}
          </span>
          <div className="flex items-baseline gap-1">
            ...
            {viewUserIds.length > 1 && (
              <span className="text-[10px] font-bold text-blue-500 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-2 py-0.5 rounded-full ml-1">
                多人模式
              </span>
            )}
          </div>
          ...
          <table className="min-w-full divide-y divide-gray-100 dark:divide-gray-800">
            <thead className="bg-white dark:bg-gray-900">
              <tr>
                ...
                <th className="px-4 py-3 text-right text-[11px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-wider w-20 sticky right-0 bg-white dark:bg-gray-900 border-l border-gray-100 dark:border-gray-800 z-10 shadow-[-4px_0_10px_rgba(0,0,0,0.02)]">
                  操作
                </th>
              </tr>
            </thead>
        ...
        <div className="p-4 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between bg-gray-50/30 dark:bg-gray-900/50">
          ...
          <div className="flex items-center gap-2">
            <Link
              ...
              scroll={false}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${safePage <= 1 ? "bg-gray-100 text-gray-300 pointer-events-none" : "bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-50"}`}
            >
              上一頁
            </Link>
            <Link
              href={`${baseUrl}&page=${safePage + 1}`}
              scroll={false}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${safePage >= totalPages ? "bg-gray-100 text-gray-300 pointer-events-none" : "bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-50"}`}
            >
              下一頁
            </Link>
          </div>
